...
export default function MenuItemCard({ item, restaurantId, restaurantName }: MenuItemProps) {
  ...
  const [selectedSize, setSelectedSize] = useState<string | undefined>(
    hasSizes ? item.sizes![0].label : undefined
  );
  ...
  function handleAdd() {
    const result = addItem(item, restaurantId, restaurantName, selectedSize);
  ...
  function handleConfirmClear() {
    confirmClearAndAdd(item, restaurantId, restaurantName, selectedSize);
    setShowConfirm(false);
  }

  // Resolve the displayed price based on selected size (or base price)
  const basePrice =
  ...
  const discountedPrice =
    item.discount_percent && item.discount_percent > 0
      ? Math.round(basePrice * (1 - item.discount_percent / 100))
      : null;
        ...
        <div className="flex-1 min-w-0 flex flex-col justify-between">
          ...
          <div className="flex items-center justify-between mt-1">
            <div className="flex items-baseline gap-1.5">
              ...
              {discountedPrice && (
                <p className="text-xs text-gray-400 line-through">${basePrice}</p>
              )}
            </div>
            ...
            {quantity === 0 ? (
              ...
              <div className="flex items-center gap-2 bg-primary/10 rounded-full px-1 py-1">
                <button
                  onClick={() => removeItem(item.id, selectedSize)}
