    ...
    <>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          {items.map(({ item, quantity, size_label, unit_price }) => (
            <div
              key={size_label ? `${item.id}::${size_label}` : item.id}
            ...
            >
              <div className="flex-1 min-w-0">
                <p className="font-medium text-gray-900 text-sm truncate">
                  {item.name}
                  {size_label ? (
                    <span className="text-gray-400 font-normal"> ({size_label})</span>
                  ) : null}
                </p>
                <p className="text-sm text-gray-500">${unit_price} each</p>
              </div>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => updateQuantity(item.id, quantity - 1, size_label)}
